...
class TaskManager:
    ...
    async def process(self, message: str, context: dict) -> AsyncGenerator[str, None]:
        ...
        for agent_name in agents_to_run:
            ...
            yield self.step(agent.emoji, f"{agent.name} working...", "running")
            ...
            yield self.step(agent.emoji, f"{agent.name} done!", "done")
        ...
        if not agent_results:
            ...
            # Single agent result — clean, no labels
            final_response = list(agent_results.values())[0]
        else:
            # Multiple agents — combine intelligently WITHOUT labels
            yield self.step("⚡", "Combining results...", "running")
            # If research + writer both ran, writer result is the main response
            if "writer" in agent_results:
                final_response = agent_results["writer"]
            elif "code" in agent_results:
                final_response = agent_results["code"]
            elif "creative" in agent_results:
                final_response = agent_results["creative"]
            elif "email" in agent_results:
                final_response = agent_results["email"]
            else:
                # Combine remaining results cleanly
        ...
        # ── Check if report — ONLY for actual report requests ──
        is_report = plan.get("is_report", False)
        if is_report and "writer" in plan.get("agents", []):
