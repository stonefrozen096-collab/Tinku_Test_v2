...
class TaskManager:
    ...
    async def process(self, message: str, context: dict) -> AsyncGenerator[str, None]:
        ...
        if enable_tp or verbose:
            thought_process.append(f"🎯 Intent: {', '.join(agents_to_run)}")
            thought_process.append(f"📋 {len(agents_to_run)} agent(s) will collaborate")
        ...
        for agent_name in agents_to_run:
            ...
            if enable_tp or verbose:
                thought_process.append(f"{agent.emoji} {agent.name}: Processing...")
            ...
            if result["success"]:
                ...
                if verbose:
                    import re as _rev
                    clean_prev = _rev.sub(r'[*_`#]+', '', result["result"])
                    preview = clean_prev[:60].replace("\n", " ").strip()
                    thought_process.append(f"📄 {agent.name} → {preview}...")
        ...
        if enable_tp or verbose:
            thought_process.append(f"⏱️ Completed in {elapsed}s")
        ...
        yield self.step("✨", "Generating response...", "done", detail="Final response streamed to you word by word.")
