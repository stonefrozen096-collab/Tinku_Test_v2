...
async def get_stock_price(query: str) -> dict:
    ...
    try:
        for k, v in CRYPTO_MAP.items():
            if k in lower:
                url = "https://api.coingecko.com/api/v3/simple/price?ids=" + v + "&vs_currencies=usd,inr&include_24hr_change=true"
                ...
                if data.get(v):
                    ...
                    result = ("💰 **" + query.upper() + " Price**\n\n"
                              "💵 USD: $" + "{:,.2f}".format(usd) + "\n"
                              "🇮🇳 INR: ₹" + "{:,.2f}".format(inr) + "\n"
                              + arrow + " 24h: " + "{:+.2f}".format(chg) + "%\n\n"
                              "*Live via CoinGecko*")
                    return {"data": result, "sources": [{"title": query.upper(), "url": "https://coingecko.com", "source": "CoinGecko"}]}
                else:
                    # Fallback: CoinCap API
                    cap_url = "https://api.coincap.io/v2/assets/" + v
                    async with httpx.AsyncClient(timeout=10) as client:
                        cap_resp = await client.get(cap_url)
                        cap_data = cap_resp.json().get("data", {})
                    if cap_data:
                        usd = float(cap_data.get("priceUsd", 0))
                        chg = float(cap_data.get("changePercent24Hr", 0))
                        arrow = "📈" if chg >= 0 else "📉"
                        result = ("💰 **" + query.upper() + " Price**\n\n"
                                  "💵 USD: $" + "{:,.2f}".format(usd) + "\n"
                                  + arrow + " 24h: " + "{:+.2f}".format(chg) + "%\n\n"
                                  "*Live via CoinCap*")
                        return {"data": result, "sources": [{"title": query.upper(), "url": "https://coincap.io", "source": "CoinCap"}]}
