...
when tool data is provided. When writing reports, be thorough and well-structured."""
...
async def web_search(query: str) -> dict:
    """Search using GNews API (real news) with Wikipedia fallback."""
    sources = []
    results = []
        ...
        async with httpx.AsyncClient(timeout=8) as client:
            ...
            if resp.status_code == 200:
                ...
                if wiki.get("extract") and len(wiki["extract"]) > 50:
                    ...
                    wiki_url_page = wiki.get("content_urls", {}).get("desktop", {}).get("page", "")
                    if wiki_url_page:
                        sources.append({"title": f"Wikipedia — {wiki.get('title', query)}", "url": wiki_url_page, "source": "Wikipedia"})
    except:
        pass

    search_text = "\n\n".join(results) if results else f"🔍 Limited results for '{query}'. Using training knowledge:"
    return {"data": "🔍 **Search Results:**\n\n" + search_text if results else search_text, "sources": sources}
...
def build_report_prompt(topic: str, search_data: str) -> str:
    """Build a structured report prompt."""
    return f"""Based on the following research data, generate a comprehensive, well-structured report about: {topic}
...
## 📝 Conclusion
[Wrap up in 2-3 sentences]

---
*Report generated by Tinku AI Agent*
*Sources: Based on latest available data*

Make it professional, informative and comprehensive. Use emojis for visual appeal."""
...
async def call_claude(messages: List[Dict], model: str, api_key: str) -> AsyncGenerator[str, None]:
    ...
    payload = {"model": model, "max_tokens": 4096, "system": SYSTEM_PROMPT, "messages": formatted}
    async with httpx.AsyncClient(timeout=60) as client:
        resp = await client.post(url, headers=headers, json=payload)
        if resp.status_code != 200:
