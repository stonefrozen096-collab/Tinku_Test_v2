...
async def analyze_sentiment(text: str, hf_key: str = None) -> dict:
    """Analyze sentiment using HuggingFace API with AI fallback."""
    ...
    if hf_key:
        try:
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.post(url, headers=headers, json={"inputs": text[:500]})
                data = resp.json()
            if isinstance(data, list) and data:
                ...
                result = (emoji + " **Sentiment: " + mood + "** (" + str(score) + "% confident)\n\n"
                         "**Analyzed text:** \"" + text[:100] + ("..." if len(text) > 100 else "") + "\"\n\n"
                         "*Powered by HuggingFace ML*")
                return {"data": result, "mood": mood.lower(), "score": score, "source": "huggingface"}
        except Exception:
            pass
    # AI fallback
...
async def summarize_text(text: str, hf_key: str = None) -> dict:
    """Summarize text using HuggingFace API with AI fallback."""
    if hf_key and len(text) > 100:
        try:
            url = "https://api-inference.huggingface.co/models/facebook/bart-large-cnn"
            headers = {"Authorization": "Bearer " + hf_key}
            payload = {"inputs": text[:1024], "parameters": {"max_length": 150, "min_length": 30}}
            ...
            if isinstance(data, list) and data and data[0].get("summary_text"):
                ...
                result = ("📝 **Summary:**\n\n" + summary + "\n\n"
                         "📊 Reduced from " + str(len(text)) + " to " + str(len(summary)) + " characters\n\n"
                         "*Powered by HuggingFace BART*")
                return {"data": result, "summary": summary, "source": "huggingface"}
        except Exception:
            pass
    # AI fallback
...
async def detect_language(text: str, hf_key: str = None) -> dict:
    ...
    if hf_key:
        try:
            ...
            headers = {"Authorization": "Bearer " + hf_key}
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.post(url, headers=headers, json={"inputs": text[:200]})
                data = resp.json()
            if isinstance(data, list) and data:
                results = data[0] if isinstance(data[0], list) else data
                results = sorted(results, key=lambda x: x.get("score", 0), reverse=True)
                top = results[0]
                lang = top.get("label", "en")
                score = round(top.get("score", 0) * 100, 1)
                return {"language": lang, "score": score, "source": "huggingface"}
        except Exception:
            pass
    return {"language": "en", "score": 0, "source": "ai"}
...
async def auto_detect_and_translate(message: str, hf_key: str = None) -> dict:
    ...
    try:
        ...
        if detected_lang != "en" and confidence > 50:
            return {
                "detected": detected_lang,
                "lang_name": lang_name,
                "confidence": confidence,
                "instruction": f"The user is writing in {lang_name}. Please understand their message and reply ENTIRELY in {lang_name}. Do not use English in your response.",
                "non_english": True
            }
