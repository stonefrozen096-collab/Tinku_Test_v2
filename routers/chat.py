...
async def send_message(data: ChatRequest, request: Request, db=Depends(get_db)):
    ...
    if is_guest:
        ...
        if current_count >= GUEST_DAILY_LIMIT:
            from fastapi.responses import JSONResponse
            return JSONResponse(status_code=429, content={"error": "guest_limit_reached", "message": f"You have used all {GUEST_DAILY_LIMIT} free messages for today. Sign up for unlimited access!", "remaining": 0})
        import datetime as _dt
        await db.guest_limits.update_one(
            {"ip": client_ip, "date": today},
            {"$inc": {"count": 1}, "$setOnInsert": {"ip": client_ip, "date": today, "created_at": _dt.datetime.utcnow()}},
            upsert=True
        )
