    ...
    async def stream():
        ...
        try:
            ...
            needs_tools = (tools["weather"] or tools["search"] or tools["currency"]
                          ...
                          or tools["pdf_export"] or tools["docx_export"]
                          or tools["note_save"] or tools["note_recall"]
                          or tools["todo_add"] or tools["todo_show"] or tools["todo_done"]
                          or tools["resume"])
            ...
            if tools["report"]:
                ...
                # ── PHASE 6 TOOLS ──
                if tools["note_save"]:
                    yield step_event("📝", "Saving note...", "running")
                    tool_context += f"\n\n{format_note_instruction(data.message)}"
                    yield step_event("📝", "Note saved!", "done")

                if tools["note_recall"]:
                    yield step_event("📝", "Fetching your notes...", "running")
                    tool_context += f"\n\n{format_recall_instruction()}"
                    yield step_event("📝", "Notes loaded!", "done")

                if tools["todo_add"]:
                    yield step_event("✅", "Adding to your tasks...", "running")
                    tool_context += f"\n\n{format_todo_instruction(data.message)}"
                    yield step_event("✅", "Task added!", "done")

                if tools["todo_show"]:
                    yield step_event("✅", "Loading your tasks...", "running")
                    tool_context += f"\n\n{format_show_todos_instruction()}"
                    yield step_event("✅", "Tasks loaded!", "done")

                if tools["todo_done"]:
                    yield step_event("✅", "Marking task done...", "running")
                    tool_context += f"\n\n{format_done_todo_instruction(data.message)}"
                    yield step_event("✅", "Task completed!", "done")

                if tools["pdf_export"] or tools["docx_export"]:
                    fmt = "PDF" if tools["pdf_export"] else "Word"
                    yield step_event("📄", f"Preparing {fmt} export...", "running")
                    tool_context += f"\n\n[FILE_EXPORT_REQUEST]\nThe user wants to export content as {fmt}. Generate the content beautifully formatted. Output: [EXPORT_FORMAT]: {fmt.lower()}"
                    yield step_event("📄", f"{fmt} export ready!", "done")

                if tools["resume"]:
                    yield step_event("📄", "Starting resume builder...", "running")
                    tool_context += f"\n\n[RESUME_BUILD]\nStart building a professional resume. Ask for name first. Output: [RESUME_START]"
                    yield step_event("📄", "Resume builder ready!", "done")
