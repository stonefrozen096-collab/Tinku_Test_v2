        ...
        try:
            ...
            if tools["report"]:
                ...
                if tools["song"]:
                    ...
                    yield step_event("🎵", "Searching for '" + q + "'...", "running")
                    sr = await search_song(q)
                    tool_context += "\n\n[SONG RESULTS]\n" + sr["data"] + "\n"
                    all_sources.extend(sr.get("sources", []))
                    yield step_event("🎵", "Searching for '" + q + "'...", "done")
                    if sr.get("songs"):
                ...
                if tools["stock"]:
                    ...
                    yield step_event("📈", "Fetching price for " + q + "...", "running")
                    stk = await get_stock_price(q)
                    tool_context += "\n\n[PRICE DATA]\n" + stk["data"] + "\n"
                    all_sources.extend(stk.get("sources", []))
                    yield step_event("📈", "Fetching price for " + q + "...", "done")
