...
from moderation import check_ban_status, add_violation, get_warning_message
...
GUEST_GEMINI_KEY = os.getenv("GUEST_GEMINI_KEY", "")
GUEST_DAILY_LIMIT = 10
GUEST_MODEL = "gemini-2.0-flash"
GUEST_PROVIDER = "gemini"
...
async def send_message(data: ChatRequest, request: Request, db=Depends(get_db)):
    ...
    print(f"DEBUG: user={user}, user_id={user_id}, is_guest={is_guest}")

    # ── Guest restrictions ──
    if is_guest:
        if not GUEST_GEMINI_KEY:
            from fastapi.responses import JSONResponse
            return JSONResponse(status_code=503, content={"error": "Guest service unavailable. Please sign in."})

        # Get client IP
        client_ip = request.headers.get("x-forwarded-for", request.client.host if request.client else "unknown")
        client_ip = client_ip.split(",")[0].strip()

        # Check daily limit in MongoDB
        from datetime import date
        today = str(date.today())
        guest_doc = await db.guest_limits.find_one({"ip": client_ip, "date": today})
        current_count = guest_doc.get("count", 0) if guest_doc else 0

        if current_count >= GUEST_DAILY_LIMIT:
            from fastapi.responses import JSONResponse
            return JSONResponse(status_code=429, content={
                "error": "guest_limit_reached",
                "message": f"You have used all {GUEST_DAILY_LIMIT} free messages for today. Sign up for unlimited access!",
                "remaining": 0
            })

        # Increment count
        await db.guest_limits.update_one(
            {"ip": client_ip, "date": today},
            {"$inc": {"count": 1}, "$setOnInsert": {"ip": client_ip, "date": today, "created_at": __import__("datetime").datetime.utcnow()}},
            upsert=True
        )
        remaining = GUEST_DAILY_LIMIT - current_count - 1

        # Force guest to use Gemini Flash with guest key
        data.provider = GUEST_PROVIDER
        data.model = GUEST_MODEL
        data.api_key = GUEST_GEMINI_KEY
        data.file_content = None
        data.file_name = None
        data.image_base64 = None
        data.image_type = None
        data.thought_process = False
        data.verbose_mode = False
    ...
    if not is_guest:
        ban_status = await check_ban_status(db, user_id)
        if ban_status["is_banned"]:
            from fastapi.responses import JSONResponse
            return JSONResponse(
                status_code=403,
                content={"error": ban_status["message"], "banned": True}
            )
    ...
    async def stream():
        ...
        try:
            ...
            # Send done event AFTER DB save so history loads correctly
            done_data = {"type": "done", "conversation_id": conv_id or ""}
            if is_guest:
                done_data["guest_remaining"] = remaining
            _done = json.dumps(done_data)
            yield f"data: {_done}\n\n"
