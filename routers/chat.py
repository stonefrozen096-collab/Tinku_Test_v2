...
def step_event(emoji: str, text: str, status: str = "running") -> str:
    ...
    _e = json.dumps({"type": "step", "emoji": emoji, "text": text, "status": status})
    return f"data: {_e}\n\n"

def sources_event(sources: list) -> str:
    """Send sources to frontend."""
    _e = json.dumps({"type": "sources", "sources": sources})
    return f"data: {_e}\n\n"
    ...
    async def stream():
        ...
        try:
            ...
            if is_flagged:
                ...
                _e = json.dumps({"type": "chunk", "text": warning})
                yield f"data: {_e}\n\n"
            ...
            async for chunk in generate_response(
                ...
                full_response += chunk
                _e = json.dumps({"type": "chunk", "text": chunk})
                yield f"data: {_e}\n\n"
            ...
            if tools["report"]:
                _e = json.dumps({"type": "report", "content": full_response})
            yield f"data: {_e}\n\n"
            ...
            if tools.get("todo_add") and "[TODO_ADDED]:" in full_response:
                ...
                if m:
                    _e = json.dumps({"type": "todo_added", "task": m.group(1), "time": m.group(2)})
                    yield f"data: {_e}\n\n"
            ...
            if tools.get("note_save") and "[NOTE_SAVED]:" in full_response:
                ...
                if m:
                    _e = json.dumps({"type": "note_saved", "note": m.group(1)})
                    yield f"data: {_e}\n\n"
            ...
            _e = json.dumps({"type": "done", "conversation_id": conv_id or ""})
            yield f"data: {_e}\n\n"
